...
static void preprocessFingerDown(SDL_Event *event) {
	...
	for (int i = 0; i < MAX_NUM_FINGERS; i++) {
		if (_finger[port][i].id == -1) {
			...
			_finger[port][i].lastX = event->tfinger.x * displayWidth;
			_finger[port][i].lastY = event->tfinger.y * displayHeight;
			break;
		}
	}
}
...
static void preprocessFingerMotion(SDL_Event *event) {
	...
	if (numFingersDown >= 1) {
		...
		if (updatePointer) {
			int x = event->tfinger.x * displayWidth;
			int y = event->tfinger.y * displayHeight;
			int xrel = 0;
			int yrel = 0;
			...
			for (int i = 0; i < MAX_NUM_FINGERS; i++) {
				if (_finger[port][i].id == id) {
					xrel = x - _finger[port][i].lastX;
					yrel = y - _finger[port][i].lastY;
